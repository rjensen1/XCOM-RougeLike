...
public class InputManagerTactical{
	...
	private static void tacticalMissionStandard(char input){
		if(currentEnvironment != null)
			currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages((int)input+"");
	...
	private static void tacticalMissionScrollPan(char input) {
		...
		if(input == ' '){
			currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("Scroll lock is on");
			tacticalMissionScrollPanLock = true;
		}
		else if(input==escapeKey){
			tacticalMissionOpenDoor = false;
			currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("Scroll lock is off");
	...
	private static void tacticalMissionOpenDoor(char input) {
		...
		if(input==' '){
			...
			currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("Open door in which direction?");
		}
		else if(input==escapeKey){
			tacticalMissionOpenDoor = false;
			currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("Cancelled open door");
		}
		else if((int)input>48 && (int)input<58){
				//false closes a door, true opens it
			currentEnvironment.thisEnvironmentTacticalUnits.getCurrentActivePCAgent().theseActions.openDoor(true, (input - 48));
			tacticalMissionOpenDoor = false;
		}
	...
	private static void tacticalMissionCloseDoor(char input) {
		...
		if(input==' '){
			...
			currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("Close door in which direction?");
		}
		else if(input==escapeKey){
			tacticalMissionCloseDoor = false;
			currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("Door closing cancelled");
		}
		else if((int)input>48 && (int)input<58){
				//false closes a door, true opens it
			currentEnvironment.thisEnvironmentTacticalUnits.getCurrentActivePCAgent().theseActions.openDoor(false, (input - 48));
			tacticalMissionCloseDoor = false;
		}
	...
	private static void inHelpScreen(char input) {
		...
		if(input==' '){
			inHelpScreen=true;
			currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("entering help");
		}
		else if(input==escapeKey){
			inHelpScreen=false;
			currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("leaving help");
		}
		...
		tacticalMissionScrollPanLockKey = 'y';
		 */
		currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages(commands);
	...
	private static void tacticalMissionTargetingWeapongOrThrow(char input) {
		...
		if(input==' '){
			...
			currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("entering targeting something");	
		}
		...
		//escape key/ cancel
			...
			currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("leaving targeting something");
		}
	}
	/*
	...
	private static void tacticalMissionLookToggle(char input) {
		...
		if(input==' '){
			...
			currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("entering look toggle");
		}
		...
		//move the look cursor
			...
			currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("leaving look toggle");
		}
	...
	private static void tacticalMissionTacticalMissionRegularInput(char input) {
		 ...
		 */
			...
			currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("End mission");
	...
	private static void tacticalMissionDropItem(char input) {
		...
		if(input==' '){
			...
			currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("Drop an item, or press escape to cancel.");	
			boolean showTUCostToGround = true;
			boolean isUnloading = false;
			currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages(currentEnvironment.thisEnvironmentTacticalUnits.getCurrentActivePCAgent().thisInventory.allInventoryToStringForDroppingMoving(showTUCostToGround, 0, isUnloading));
		}
		
		else if(input==escapeKey){
			tacticalMissionDroppingAnItem=false;
			currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("leaving drop item");	
		}
			//if it's between a and z
		else if(((int)input - 97 > -1) && ((int)input - 122 < 1)){
			currentEnvironment.thisEnvironmentTacticalUnits.getCurrentActivePCAgent().thisInventory.dropThisItem((int)input - 97);
			tacticalMissionDroppingAnItem=false;
	...
	private static void tacticalMissionUnloadWeapon(char input) {
		...
		if(input==' '){
			...
			currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("entering weapon unloading");	
			boolean showTUCostToGround = false;
			boolean isUnloading = true;
			currentEnvironment.thisEnvironmentTacticalUnits.getCurrentActivePCAgent().thisInventory.allInventoryToStringForDroppingMoving(showTUCostToGround, 0, isUnloading);
		}
			...
			tacticalMissionUnloadingAWeapon = false;
			currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("leaving weapon unloading");	
		}
		//if it's between a and z
		else if(((int)input - 97 > -1) && ((int)input - 122 < 1)){
			...
			if(!getMoveItemListed){
					//list the objects
				boolean isUnloading = true;
				currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages(currentEnvironment.thisEnvironmentTacticalUnits.getCurrentActivePCAgent().thisInventory.whereThisItemCanGoToString(onGround, (int)input - 97, page, isUnloading));
				...
				currentEnvironment.thisEnvironmentTacticalUnits.getCurrentActivePCAgent().thisInventory.moveItemInInventory(tempItemNumToPickupOrMove, tempPlaceToPutObject, itemOnGround, itemIsLoaded);
				tacticalMissionUnloadingAWeapon = false;
				currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("Done");
				currentEnvironment.displayTacticalEnvironment(0);
				getMoveItemListed = false;
	...
	private static void tacticalMissionMoveItem(char input) {
		...
		if(input==' '){
			...
			currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("entering move item");	
			boolean showTUCostToGround = false;
			boolean isUnloading = false;
			currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages(currentEnvironment.thisEnvironmentTacticalUnits.getCurrentActivePCAgent().thisInventory.allInventoryToStringForDroppingMoving(showTUCostToGround, 0, isUnloading));
		}
			...
			getMoveItemListed = false;
			currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("leaving move item");	
		}
		//if it's between a and z
		else if(((int)input - 97 > -1) && ((int)input - 122 < 1)){
	...
	private static void tacticalMissionGetItem(char input) {
		...
		if(input==' '){
			...
			currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("Get item from ground:");	
			int page = 0;
			currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages(currentEnvironment.thisEnvironmentTacticalUnits.getCurrentActivePCAgent().thisInventory.showItemsOnGroundWithAlpha(page));
		}
	...
	private static void tacticalMissionDeselectUnit() {
		...
		currentEnvironment.thisEnvironmentTacticalUnits.selectNextPCUnitDeselectCurrent();
		currentEnvironment.displayTacticalEnvironment(0);
	}
	/*
	...
	private static void tacticalMissionCycleUnitBackward() {
		...
		currentEnvironment.thisEnvironmentTacticalUnits.selectPreviousUnitPC();
		currentEnvironment.displayTacticalEnvironment(0);
	...
	private static void tacticalMissionCycleUnitForward() {
			...
			//next
		currentEnvironment.thisEnvironmentTacticalUnits.selectNextUnitPC();
	...
	private static void tacticalMissionViewInventoryScreen(char input) {
		...
		if(input==' '){
			...
			currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("Inventory: "+tacticalMissionGetKey+": To pick up an item, "+tacticalMissionDropKey+": Drops an item, \n"+tacticalMissionMoveKey+": To move an item, "+tacticalMissionUnloadKey+": to unload a wielded weapon, "+escapeKey+": If done.\n");
			currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages(currentEnvironment.thisEnvironmentTacticalUnits.getCurrentActivePCAgent().thisInventory.inventoryToString());
			
		}
		else if(input == tacticalMissionGetKey){
			currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("In inventory: Getting Item From Ground");
			...
			currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("In inventory: Dropping item to ground");
			...
			currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("In inventory: Moving item in inventory");
			...
			currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("In inventory: Unloading a wielded weapon");
			...
			tacticalMissionDroppingAnItem=false;
			tacticalMissionUnloadingAWeapon = false;
			currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("Leaving inventory Screen");	
		}
	...
	private static void tacticalMissionStealthToggle() {
		...
		currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("Toggle Stealth");
		currentEnvironment.thisEnvironmentTacticalUnits.getCurrentActivePCAgent().theseStats.toggleStealthOn();
		currentEnvironment.displayTacticalEnvironment(0);
	...
	private static void tacticalMissionStanceDown() {
		 ...
		 */
		// TODO Auto-generated method stub
		currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("stance down");
		currentEnvironment.thisEnvironmentTacticalUnits.getCurrentActivePCAgent().theseStats.stanceDown();
		currentEnvironment.displayTacticalEnvironment(0);
	}
	/*
	...
	private static void tacticalMissionStanceUp() {
		 ...
		 */
		// TODO Auto-generated method stub
		currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("stance up");
		currentEnvironment.thisEnvironmentTacticalUnits.getCurrentActivePCAgent().theseStats.stanceUp();
		currentEnvironment.displayTacticalEnvironment(0);
	}
	...
	if(input==' '){
		tacticalMissionUsingAnItem=true;
		currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("entering use item");	
	}
	else if(input==escapeKey){
		tacticalMissionUsingAnItem=false;
		currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("leaving use item");	
	}
	...
	private static void tacticalMissionMoveUnitUpDownStairs(char input) {
		...
		currentEnvironment.thisEnvironmentTacticalUnits.getCurrentActivePCAgent().thisMovement.upDownStairs(sendVal/*1 = down, 0 = up*/);
		currentEnvironment.displayTacticalEnvironment(0);
	}
	/*
	...
	private static void tacticalMissionMoveLookCursor(int i) {
		currentEnvironment.thisEnvironmentTacticalUnits.getCurrentActivePCAgent().thisMovement.lookDirectionChange(i, false);
		currentEnvironment.displayTacticalEnvironment(0);
	...
	private static void tacticalMissionMoveUnit(int i) {
		...
		if(currentEnvironment.thisEnvironmentTacticalUnits.getCurrentActivePCAgent() != null){
			currentEnvironment.thisEnvironmentTacticalUnits.getCurrentActivePCAgent().thisMovement.moveUnitDirectionNumpad(i);
			currentEnvironment.displayTacticalEnvironment(0);
			currentEnvironment.thisEnvironmentTacticalUnits.addToUnitMessages("move: "+i);
		}
	}
