...
The safety palette is made up of 216 colors that result from all the possible combinations of the values 0, 51, 102, 153, 204, and 255 (or $00, $33, $66, $99, $CC, and $FF in hex notation) for the red, green and blue components. For example, the color RGB(204,153,102) or $6699CC is one of the safe colors.
	...
	public static DisplayColorChars[][] makeColorCharComposite(char[][] repChar,  Color tempFront, Color tempBack) {
		// makes a composite of the colors etc
		DisplayColorChars[][] retVal = new DisplayColorChars[repChar.length][repChar[0].length];
		//will not support ragged
		//TODO
		for(int y = 0; y < repChar[0].length; y++){
			for(int x=0; x < repChar.length; x++){
				retVal[x][y] = new DisplayColorChars(repChar[x][y], tempFront, tempBack);
			}
		}
		return retVal;
	}
