...
public class AgentInventory {
	...
	public String allInventoryToStringForDroppingMoving(boolean showTUCostToGround, int pageNumber, boolean isUnloading) 
	{
		...
		{		//if there is at least one item in this slot, print it out along with it's TU drop cost
			if( ! (unitCubbyLLArray[compartment].size() < 1)){
				...
				if(isUnloading)
					TUcost = actor.getTheseCosts().getTUCostUnloading();
				...
				if( (!actor.getTheseStats().TUHas(TUcost)) && (showTUCostToGround || isUnloading) )
					TUcostString +="**";
				if(isUnloading)
	...
	public void dropThisItem(int itemToDrop){
		...
		for(int compartment = 0; compartment < unitCubbyLLArray.length; compartment ++){
			...
			while(tempIt.hasNext()){
				if(alpha == itemToDrop){
					...
					if(actor.getTheseStats().TUHas(  TUCostInventoryMoveGet(compartment, groundCompartmentIntNumberGet()))  ){
						actor.getTheseMessages().errorMessageAdd("Dropping "+ tempObToDrop.toString());
						tempIt.remove();
						actor.currentEnvironment.getThisEquipment().dropItemToGround(tempObToDrop, tempxyz[0], tempxyz[1], tempxyz[2]);
						dropped = true;
						actor.getTheseStats().TUUse( TUCostInventoryMoveGet(compartment, groundCompartmentIntNumberGet()) );
						actor.refreshHUD();
					}
					else{
						actor.getTheseMessages().errorMessageAdd("Not enough TUs");
						dropped = true;
					}
				}
		...
		if(!dropped)
			actor.getTheseMessages().errorMessageAdd("Invalid item");
	}
	...
	public String whereThisItemCanGoToString(boolean isOnGround, int itemNumber, int pageNumber, boolean isUnloading) 
	{
		...
		if(isOnGround && actor.currentEnvironment.getThisEquipment().getNumberOfObjectsAtThisLocation(xyzTemp[0], xyzTemp[1], xyzTemp[2]) > itemNumber*(pageNumber+1))
		{
					//getting a size/id twin
			sizeID = actor.currentEnvironment.getThisEquipment().getSizeIDTypeItemFromGroundAtLocationAndIntPostionInList(xyzTemp[0], xyzTemp[1], xyzTemp[2], itemNumber*(pageNumber+1));
		...
		for(int compartment = 0; compartment < unitCubbyLLArray.length; compartment++)
		{
			...
			if(!showLoadStuff){
				...
				int TUcost = actor.getTheseCosts().getTUCostUnloading();
				if(!isUnloading)
				...
				String TUcostString = "";
					...
					//if it's too expensive, mark it with asterix
				if( !actor.getTheseStats().TUHas(TUcost))
				{
					TUcostString +="**";
					impossible = true;
				}
	...
	public String showItemsOnGroundWithAlpha(int page) {
		...
		int[] xyzTemp = actor.locationXYZTripletGet();
		return actor.currentEnvironment.getThisEquipment().getItemGroundNameList(xyzTemp[0], xyzTemp[1], xyzTemp[2], prefixWithAlphaCharacters, page);
	...
	public void moveItemInInventory(int tempItemNumToPickupOrMove, int tempPlaceToPutObject, boolean sourceIsGround, boolean itemIsLoaded) {
		...
		if(!sourceIsGround){
			...
			if(sizeID[2] == 1 && tempPlaceToPutObject < this.numCompartmentsWieldable){
				...
				if(unitCubbyLLArray[tempPlaceToPutObject].size()!=0 && unitCubbyLLArray[tempPlaceToPutObject].peekFirst().doesItTakeThisAmmo(sizeID[1])){
					...
					TUcost = actor.getTheseCosts().getTUCostLoading();
					}
					else
						actor.getTheseMessages().errorMessageAdd("Weapon is already loaded.");
				}
				else
					actor.getTheseMessages().errorMessageAdd("Weapon doesn't take this ammo.");
			}
			...
			if(inRange){
				...
				enoughTU = actor.getTheseStats().TUHas(TUcost);
			...
			if(foundItemOnPerson && enoughTU && ((enoughSpace && itemNotTooBig) || areWeLoadingThisIntoAWeapon)){
				...
				if(!areWeLoadingThisIntoAWeapon)
					...
					actor.getTheseMessages().errorMessageAdd("Ammo Loaded");
				}
				actor.getTheseStats().TUUse(TUcost);
			}
			else{
				//errors!
				if(!inRange)
					actor.getTheseMessages().errorMessageAdd("Cannot put it there.");
				else if(!foundItemOnPerson)
					actor.getTheseMessages().errorMessageAdd("Invalid object");
				else if(!enoughTU)
					actor.getTheseMessages().errorMessageAdd("Not enough TUs");
				else if(!enoughSpace)
					actor.getTheseMessages().errorMessageAdd("Not enough free space");
				else if(!itemNotTooBig)
					actor.getTheseMessages().errorMessageAdd("Item is too large");
			}
			...
			int[] xyzTemp = actor.locationXYZTripletGet();
			toMove = actor.currentEnvironment.getThisEquipment().getItemFromGroundAtLocationAndIntPostionInList(xyzTemp[0], xyzTemp[1], xyzTemp[2], tempItemNumToPickupOrMove);
			if(toMove !=null)
				found = true;
				
		
		if(found && actor.getTheseStats().TUHas(TUcost)){
		...
		if(toMove != null){
			...
			if(!sourceIsGround)
				...
				actor.currentEnvironment.getThisEquipment().dropItemToGround(toMove, xyzTemp[0], xyzTemp[0], xyzTemp[0]);
			}
		}
		else
			actor.getTheseStats().TUUse(TUcost);
