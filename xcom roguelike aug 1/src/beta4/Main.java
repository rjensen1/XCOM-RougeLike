...
public class Main{
	...
	private static void newMission() {
		...
		PackageShip tempShip = new PackageShip(1, null);
		...
		tempShip.thisShipsEnvironmentChange(currentTacticalEnvironment);
		...
		currentTacticalEnvironment.getCurrentActivePCAgent().getThisInventory().equipThisObject(tempOb, 1);
		
		//currentTacticalEnvironment.getCurrentActivePCAgent().getEquipObAtSlotLocationInCompartment(0, 1);
			//connecting the environment to its input manager
		//InputManager.setEnvironment(currentTacticalEnvironment);
		
			//connecting the environment to its display
		//currentTacticalEnvironment.setDisplay(display);
