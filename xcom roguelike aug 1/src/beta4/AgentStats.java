	...
	public boolean stanceUp(){
 		...
 		if(stance >= stanceMinForMobility && stance < stanceNameStringArray.length-2){
 			...
 			if(stance != origStance){
 	 			...
 	 			boolean error = false;
 	 			int ENCost = actor.getTheseCosts().getStanceENCostSwitchUpArray(stance);
 	 			int TUCost = actor.getTheseCosts().getStanceTUCostSwitchUpArray(stance);
 	 			...
 	 			if(TUHas(TUCost)&& ENHas(ENCost)){
					...
					actor.getTheseMessages().errorMessageAdd("Not enough TU/enegry to increase stance");
 					error=true;
 				}
 			...
 			actor.getTheseMessages().errorMessageAdd("You're dead or unable to move");
 		else 
 			actor.getTheseMessages().errorMessageAdd("You can't get any faster");
 	...
 	public boolean stanceDown(){
 		...
 		if(stance > stanceMinForMobility){
 			...
 			actor.getTheseMessages().errorMessageAdd("You're dead or unconscious");
 		else 
 			actor.getTheseMessages().errorMessageAdd("You can't get any lower.");
 		...
 		if(stance!=origStance){
 			...
 			int ENCost = actor.getTheseCosts().getStanceENCostSwitchDownArray(stance);
	 		int TUCost = actor.getTheseCosts().getStanceTUCostSwitchDownArray(stance);
	 		...
	 		if(TUHas(TUCost)&& ENHas(ENCost)){
					...
					actor.getTheseMessages().errorMessageAdd("Not enough TU/enegry to lower stance");
					error=true;
				}
 			if(!error){
 	 			currentTimeUnits = tempTU;
