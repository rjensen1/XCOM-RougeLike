...
public class Species {
	...
	public HasAgency intializeThisAgent(HasAgency thisOne){
		HasAgencyBirthing.thisAgentPersonalBiography(thisOne);
		
		{
			thisOne.getTheseCosts().setTUStanceMoveCost(TUStanceMoveCost);
			thisOne.getTheseStats().setStanceNameStringArray(stanceNameStringArray);
			thisOne.getTheseCosts().setENStanceMoveCost(ENStanceMoveCost);
			thisOne.getTheseStats().setStanceMinForMobility(stanceMinForMobility);
			
			thisOne.getTheseCosts().setStanceENCostSwitchUpArray(stanceENCostSwitchUpArray);
			thisOne.getTheseCosts().setStanceTUCostSwitchUpArray(stanceTUCostSwitchUpArray);
				//cost to move to this stance from a higher stance
			thisOne.getTheseCosts().setStanceENCostSwitchDownArray(stanceENCostSwitchDownArray);
			thisOne.getTheseCosts().setStanceTUCostSwitchDownArray(stanceTUCostSwitchDownArray);
			thisOne.getTheseStats().setStanceHeightModArray(stanceHeightModArray);
			thisOne.getTheseStats().setStanceAvailable(stanceAvailable);
			thisOne.getThisVision().setStanceFOVAffectArray(stanceFOVAffectArray);
			thisOne.getTheseStats().setStance(stance);
			thisOne.getTheseStats().setMaxTimeUnits(maxTimeUnits);
			thisOne.getTheseStats().setThisAgentSex(thisAgentSex);
			
			thisOne.getTheseStats().setStealthOn(stealthOn);
			// have field of view be affected by stance, IE, sprinting might give tunnel vision, while kneeling/standing might give considerably more.
			thisOne.getThisVision().setAgentFOVHoriz(agentFOVHoriz);
			...
			//agentTempSkills; //would would temporary things, like blindness, limping, etc.
			 //holds all the linkedlists for equipment
			thisOne.getThisInventory().setInventorySlotNames(inventorySlotNames);
			thisOne.getThisInventory().setNumCompartmentsWieldable(numCompartmentsWieldable);
			thisOne.getThisInventory().setEquipMaxSizeArray(equipMaxSizeArray);
			thisOne.getThisInventory().setEquipMaxNumberArray(equipMaxNumberArray);
			thisOne.getThisInventory().setUnitCubbyLLArray(unitCubbyLLArray);
			thisOne.getThisInventory().setUnitMoveItemCostArray(unitMoveItemCostArray);
			thisOne.heightCMSet(heightCM);
			thisOne.getTheseStats().setMaxEnergy(maxEnergy);
			thisOne.getTheseStats().setMaxHealth(maxHealth);
			thisOne.getTheseStats().setMaxStrength(maxStrength);
			thisOne.getThisVision().setCanRemember(canRemember);
			thisOne.getThisVision().setCanSeeOtherUnitsVisionAlways(canSeeOtherUnitsVisionAlways);
			thisOne.getTheseStats().setExperience(experience);
			thisOne.getTheseStats().setRank(rank);
			thisOne.getThisVision().setSightDistance(sightDistance);
			thisOne.getTheseStats().setRankArray(rankArray);	//holes the number of compartments (should always be the first ones) that are 'wieldable'
			thisOne.getThisInventory().setNumCompartmentsWieldable(numCompartmentsWieldable);
			thisOne.getTheseCosts().setTUCostUnloading(TUCostUnloading);
			thisOne.getTheseCosts().setTUCostLoading(TUCostLoading);
			thisOne.representingCharSet(representingChar);
			thisOne.representingColorFrontSet(standardColorFront);
			thisOne.representingColorBackSet(standardColorBack);
			thisOne.thisNameSet(thisName);
			thisOne.dimensionsSet(dimensions);
